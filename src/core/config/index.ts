...
import { CONFIG_BUILD_PAGE_URL_FORMAT, CONFIG_BUILD_ROBOTS_TXT, CONFIG_BUILD_SITEMAP, CONFIG_DEVOPTIONS_HOSTNAME, CONFIG_DEVOPTIONS_PORT, CONFIG_DEVOPTIONS_USETLS, CONFIG_DIST_FOLDER, CONFIG_HOOKS_FOLDER, CONFIG_PAGES_FOLDER, CONFIG_PUBLIC_FOLDER, CONFIG_ROOT_FOLDER, CONFIG_SRC_FOLDER } from "./defaults"

export const getExecutionMode = (): Mode => 
    (process.env.NODE_ENV || "development") as Mode
...
export const finalizeConfig = (config: Config) => {
    ...
    if (!config.buildOptions!.site) {
        config.buildOptions!.site = 
            `${config.devOptions?.useTls ? 
                'https' : 'http'}://${config.devOptions?.hostname}`
    }
    ...
    if (config.buildOptions!.sitemap) {
        (config.buildOptions!.robotsTxt as RobotsTxtOptions).sitemap = 
            config.buildOptions!.site + '/sitemap.xml'
    }
}