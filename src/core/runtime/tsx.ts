...
/** filters comments and undefines like: ['a', 'b', false, {}] to: ['a', 'b', false] */
 // @ts-ignore
const filterComments = (children: any) =>
  children.filter((child: any) => !isJSXComment(child));
...
const runtimeAttachEventHandlers = (attributes: any) => {
  ...
  Object.keys(attributes).forEach(arributeName => {
    ...
    attributes[arributeName] = 
      `((e) => Vanil.e('${attributes[arributeName]}', e))(arguments[0])`
  })
}
 ...
 */
Vanil.tsx = (type: string | Function | undefined, attributes: IVirtualNodeAttributes, ...children: IVirtualChildren): VDOMNode => {
