...
export const createVirtualDomScriptElement = 
    (document: Document, scriptCode: string, attributes?: { [name: string]: string }) => {
    ...
    scriptEl.appendChild(
        document.createTextNode(scriptCode)
    )
    return scriptEl
}
...
export const materializeDOM = async(
    executionResult: ExecutionResult<VDOMNode, any>, 
    context: Context) => {
    ...
    if (actualTopLevelElement?.localName === 'html') {
        ...
        await injectInteractiveRuntimeLibrary(document, document.head, context, runtimeLibraryFeatureFlags, executionResult.state)
    }
    return replaceStyleReplacements(
        uncapeCurlyBracketsAndBackticks(document.toString()),
        context
    )
}