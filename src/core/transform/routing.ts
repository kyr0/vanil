...
export const materializeDynamicRoutingPaths = async(context: Context) => {
    ...
    if (!isolatedGetStaticPathsFnCode) {
        ...
        throw new Error(`The following path is dynamic: ${
    ...
    const transpiledCode = (await transpileSSGCode(isolatedGetStaticPathsFnCode as string, context))
    ...
    try {
        result.data?.forEach((pageParamsAndProps, index) => {
            ...
            paramNames.forEach(paramName => {
                ...
                if (!paramValue) {
                    throw new Error(
                        `getStaticPaths() returned the following params: ${
                            JSON.stringify(pageParamsAndProps.params, null, 2)
...
export const getPathUri = (materializedPath: string, context: Context) => {
    ...
    let relativePathUriEncoded = relativePathNoFileExt.split('/')
