...
export const transformSingle = async(context: Context) => {
    ...
    try {
        ...
        html = renderSSGErrorReport(toProjectRootRelativePath(context.path!, context.config), {
...
export const transformVanilImports = (scriptCode: string) => {
    ...
    const importStmts = parseImportStatements(scriptCode, 
        false /* include await import() dynamic imports too */)
