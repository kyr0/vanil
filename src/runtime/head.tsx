import {
  ...
  Match,
  onCleanup,
  ParentComponent, sharedConfig, Switch, useContext
} from "solid-js";
...
export function renderTagsClient(tags: Array<TagDescription>) {

    return (
        <For each={tags}>
            {(tag, i) => <Switch>
                <Match when={tag.tag === 'title' || tag.tag === 'style'}>
                    <title data-sm={tag.id} {...tag.props} textContent={Array.isArray(tag.props.children) ? tag.props.children.join("") : String(tag.props.children)}></title>
                </Match>
                <Match when={tag.tag === 'meta'} >
                    <meta data-sm={tag.id} {...tag.props} />
                </Match>
                <Match when={tag.tag === 'link'} >
                    <link data-sm={tag.id} {...tag.props} />
                </Match>
                <Match when={tag.tag === 'base'} >
                    <base data-sm={tag.id} {...tag.props} />
                </Match>
            </Switch>}
        </For>
    )
